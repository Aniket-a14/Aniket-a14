#!/usr/bin/env python3
"""
aniket-a14 — terminal profile promo.

Adapted from github.com/guilyx/guilyx (assets/promo/generate.py). That repo
carries no LICENSE file, so this reuse has no explicit grant — credit stays
here regardless. The scene architecture, easing, glyph rain, bloom and
adaptive-palette encoder are theirs; the content below (identity, pipeline,
project graph, trajectory, sign-off) has been rewritten for this profile.

Renders `aniket-terminal.gif`: a ~36s looping sequence that opens as a
terminal, answers `whoami` with the identity card, ticks a pipeline tree, runs
a project graph and lets three of its nodes settle into a mark, traces the
trajectory, and signs off.

The subject is a RAG-style agent pipeline underneath, and above it the graph
of what's actually shipped: one hub project, its siblings, and the plan
between them.

Palette and the accent budget: the accent is the only saturated value in the
system, so it is spent once per scene and nowhere else. Everything else lives
on the neutral ramp, which is what lets a single accent read as "this is the
live path".
...
OUT_WEBP = os.path.join(HERE, "aniket-terminal.webp")

RNG = random.Random(14)    # fixed seed: regeneration is reproducible
...
# --------------------------------------------------------------------------
# Palette — near-black ground, one violet accent spent once per scene
# --------------------------------------------------------------------------
...
# --------------------------------------------------------------------------
# Type — a two-register system:
#   lowercase mono  = the machine talking
#   sentence-case display = the person
# --------------------------------------------------------------------------

# macOS system fonts: Menlo (mono) and Helvetica (display) are both .ttc
# collections, so faces are picked by index rather than by separate paths.
MONO_PATH = "/System/Library/Fonts/Menlo.ttc"
MONO_REG, MONO_BOLD = 0, 1
DISP_PATH = "/System/Library/Fonts/Helvetica.ttc"
DISP_REG, DISP_BOLD = 0, 1
...
F_MONO_XS = font(MONO_PATH, 10, MONO_REG)
F_MONO_S = font(MONO_PATH, 12, MONO_REG)
F_MONO = font(MONO_PATH, 13.5, MONO_REG)
F_MONO_B = font(MONO_PATH, 13.5, MONO_BOLD)
F_MONO_L = font(MONO_PATH, 17, MONO_REG)
F_MONO_XL = font(MONO_PATH, 30, MONO_BOLD)
F_DISP_XL = font(DISP_PATH, 54, DISP_BOLD)
F_DISP_L = font(DISP_PATH, 34, DISP_BOLD)
F_DISP_M = font(DISP_PATH, 21, DISP_REG)
...
# running together. Modelling a RAG pipeline as a behaviour tree: check the
# cache before paying for a reparse, then retrieve and rank together. The
# statuses below are what this tree actually returns — `reparse` is never
# reached because `cache hit` succeeds first.
# --------------------------------------------------------------------------
...
BT_NODES = {
    ...
    "battery": (185.0, 244.0, "\u25cb", "cache hit"),
    "rtl":     (355.0, 244.0, "\u25b8", "reparse"),
    "track":   (525.0, 244.0, "\u25b8", "retrieve"),
    "stream":  (695.0, 244.0, "\u25b8", "rank chunks"),
}
...
# Project graph
#
# What's actually shipped, in one picture: one hub project and the siblings
# that share its stack. Messages travel the edges; the nodes are the repos.
# --------------------------------------------------------------------------

GRAPH_NODES = [
    ("wizard", 236.0, 140.0),
    ("ai-friend", 158.0, 258.0),
    ("resume-analyzer", 348.0, 318.0),
    ("sra", 528.0, 132.0),
    ("crime-analysis", 712.0, 232.0),
    ("soulspace", 588.0, 330.0),
]
...
# --------------------------------------------------------------------------
# The mark — three shipped projects holding a formation.
# Geometry is on a 32x32 grid, about the centroid.
# --------------------------------------------------------------------------
...
def status_bar(d, k, label):
    ...
    text(d, W - 28, H - 24, "aniketdesign.in", F_MONO_XS, fade(FAINT, k), anchor="ra")
...
# --------------------------------------------------------------------------
# Content — every string below traces back to real GitHub data (repos,
# stars, contributions) or the profile README. Machine register is
# lowercase; the person is sentence case.
# --------------------------------------------------------------------------

TERM_LINES = [
    # (start, indent, segments[(text, colour)], typed?)
    (0,  0, [("aniket-a14", MUTED), (" on ", FAINT), ("main", MUTED),
             (" [wizard] ", FAINT), ("took 9s", FAINT)], False),
    (2,  0, [("→ ", ACCENT), ("ssh aniket@aniketdesign.in", BODY)], True),
    (17, 1, [("handshake ", FAINT), ("·" * 14, LINE), (" ok", BODY)], False),
    (20, 1, [("locale ", FAINT), ("·" * 17, LINE),
             (" india · utc+05:30", BODY)], False),
    (22, 0, [("→ ", ACCENT), ("whoami", BODY)], True),
]

# The joke, and it is true: temperature=0 does not make an LLM deterministic —
# floating point and batched inference see to that — and every agent here
# ships anyway. One joke, and it has to be true, so this is the one, and it
# sets up the pipeline scene after it.
JOKE_LINES = [
    (0,  0, [("aniket-a14", MUTED), (" on ", FAINT), ("main", MUTED),
             (" [wizard]", FAINT)], False),
    (2,  0, [("→ ", ACCENT), ("wizard --temperature 0", BODY)], True),
    (15, 1, [("still not bit-for-bit deterministic.", BODY)], False),
    (18, 1, [("shipping it anyway.", MUTED)], False),
]

# Every value below is real: the role line, the stack, and the projects the
# pipeline and graph scenes go on to draw.
SPEC = [
    ("role", "ai engineer · agents & full-stack"),
    ("based", "india"),
    ("building", "wizard · local-first data agent"),
    ("also", "rag pipelines · full-stack tools"),
]

TRAJECTORY = [
    (2022.6, "LPU", "b.tech cse"),
    (2024.8, "SRA", "shipped · 22 stars"),
    (2026.2, "Wizard-AIA", "founded org"),
    (2026.6, "Wizard", "local-first agent"),
]
TRAJ_T0, TRAJ_T1 = 2022.0, 2026.9
...
def scene_identity(d, f):
    ...
    if t >= 1:
        text(d, x0, 78, "Aniket Saha", F_DISP_XL, HEADING)
    ...
    if t >= 5:
        n = int((t - 5) * 3.4)
        l1 = "One request, four stages."
        l2 = "Answers you can trace to source."
...
def scene_bt(d, f):
    ...
    if t >= 1:
        ...
        n = int((t - 1) * 1.9)
        text(d, 54 + 2 * CW, 52, "pipeline tick --query policy.pdf"[:n], F_MONO, BODY)
    ...
    if t >= 32:
        ...
        ctext(d, W / 2, 312, "the tick descends. the cache answers first.",
              F_MONO_S, fade(MUTED, a))
    if t >= 34:
        a = ease_out(seg(t, 34, 4))
        ctext(d, W / 2, 334, "agent pipeline \u00b7 request lifecycle",
...
def scene_graph(d, f, positions, alive):
    ...
    if t >= 1:
        text(d, 54, 52, "\u2192 ", F_MONO, fade(ACCENT, 0.9))
        n = int((t - 1) * 1.9)
        text(d, 54 + 2 * CW, 52, "ls -la projects/"[:n], F_MONO, BODY)

    draw_graph(d, t, ease_out(seg(t, 5, 6)), positions, alive)

    if t >= 24:
        a = ease_out(seg(t, 24, 5))
        ctext(d, W / 2, 314, "shipped, or it does not count.",
              F_MONO_S, fade(MUTED, a))
    if t >= 27:
        a = ease_out(seg(t, 27, 4))
        ctext(d, W / 2, 336,
              "side projects \u00b7 one graph of them",
              F_MONO_XS, fade(FAINT, a * 0.9))
...
def scene_trajectory(d, f):
    ...
    n = int(max(0, t) * 2.4)
    text(d, 54 + 2 * CW, 54, "trajectory --since 2022"[:n], F_MONO, BODY)
    ...
    if t >= 20:
        a = ease_out(seg(t, 20, 6))
        ctext(d, W / 2, H - 66,
              "2,854 commits · 3,354 contributions in 2026 · 47 public repos",
...
def scene_sign(d, f):
    ...
    if t >= 3:
        ...
        ctext(d, cx, 206, "aniket-a14", F_MONO_XL, fade(HEADING, b))

    if t >= 5:
        b = ease_out(seg(t, 5, 4))
        ctext(d, cx, 256, "Aniket Saha — ai engineer, agents & full-stack",
    ...
    if t >= 9:
        ...
        left = "github.com/Aniket-a14"
        right = "aniketdesign.in"
        gap = 4.0
    ...
    if t >= 11 and (f // 4) % 2 == 0:
        half = text_w("aniket-a14", F_MONO_XL) / 2.0
        cwx = cw(F_MONO_XL)
        x = cx + half + cwx * 0.35
